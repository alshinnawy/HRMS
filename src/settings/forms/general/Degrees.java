...
//        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("الدرجة العلمية");
    ...
    private void searchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchActionPerformed
        ...
        if(name.getText().length()>0 &&name.getText().length()<=100)
        {
            ...
            String  degreeNames=name.getText().trim();
            DegreeHandler dh=new DegreeHandler();
            try {
                DefaultTableModel dtm =(DefaultTableModel) table.getModel();
                dtm.setRowCount(0);
                Degree[] list =dh.getDegreeByName(degreeNames);
                ...
                if(list !=null &&list.length>0)
                {
                    for(int i=0;i<list.length;i++)
                    {
                        ...
                        newRow.addElement(list[i].getDegereeName());
                        newRow.addElement(list[i].getDegreeId());
                        dtm.addRow(newRow);
                    }
                    table.setModel(dtm);
                    table.updateUI();
    ...
    private void addActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addActionPerformed
        ...
        if(name.getText().length()>0 &&name.getText().length()<=100)
        {
            ...
            dg.setDegereeName(name.getText().trim());
            DegreeHandler dh=new DegreeHandler();

            try {
                DefaultTableModel dtm =(DefaultTableModel) table.getModel();
                dtm.setRowCount(0);
                DegreePk dpk= dh.addDegree(dg);

                JOptionPane.showMessageDialog(null, "تمت الاضافة ");
                  viewAll();
                 name.setText("");
                 
//                Degree[] list =dh.getDegreeAll();
//                if(list !=null &&list.length>0)
//                {
//                    for(int i=0;i<list.length;i++)
//                    {
...
//                }
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        ...
        if(selected >=0)
        {
             Degree dg=new Degree();
             DegreeHandler dh=new DegreeHandler();
             DegreePk dpk=new DegreePk((int)table.getValueAt(selected,1));
              dg.setDegreeId(dpk.getDegreeId());
            try {
                dh.deleteDegree(dpk);
    ...
    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        ...
        if(selected >=0 && newname.getText().length()>0 &&newname.getText().length()<=100)
        {
             ...
             DegreeHandler dh=new DegreeHandler();
             DegreePk dpk=new DegreePk((int)table.getValueAt(selected,1));
             dg.setDegereeName(newname.getText().trim());
             dg.setDegreeId(dpk.getDegreeId());
             
            try {
                dh.updateDegree(dpk, dg);
    ...
    private void cancelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cancelActionPerformed
      ...
      private void viewAll() throws DegreeDaoException
    {
    DegreeHandler dh=new  DegreeHandler();
      DefaultTableModel dtm =(DefaultTableModel) table.getModel();
                  dtm.setRowCount(0);
